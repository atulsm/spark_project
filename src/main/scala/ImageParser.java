 ...
 */
  ...
  toGray(image);
  System.out.println("created resize file");
  File output = new File(file.toString());
  ImageIO.write(image, extension, output);
  System.out.println("created resize file written");
  }

  try
  {
  /* args[2] is the Class of the image, Class = Male/Female. Vector will be written into a text file */
  try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("C:/spark-example/Training_data-"+args[2]+".csv", true)))) {
